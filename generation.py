...
class Encoder(nn.Module):
    def __init__(self, channels, ch=224, z=4000):
...
class VAE(nn.Module):
    def __init__(self, channel_in, z=4000):
...
def train(data_loader, model, optimizer, device_use, beta):
    ...
    for idx in range(len(data_loader)):
        ...
        #data_rs.to(device_use)
        torch.device('cuda')

        optimizer.zero_grad()
        recon_batch, mu, logvar = model.forward(data_rs)
...
def reconstruction(original_image,model):
    ...
    :return:
    '''
    rec_image, mu, logvar = model.forward(original_image,Train=False)
    return rec_image
